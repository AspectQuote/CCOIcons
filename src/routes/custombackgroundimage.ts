...
const whitelistedFilterEffectsForBackgrounds: filterID[] = [
    "dotmatrix",
    "bside",
    "rotatedscreentone",
    "twotone",
...
const route: CCOIcons.documentedRoute = {
    ...
    responseFunction: async (req, res) => {
        ...
        if (!filterIDs.includes(filterName) || !whitelistedFilterEffectsForBackgrounds.includes(filterName)) return res.json({ success: false, message: "Invalid filter supplied." });
